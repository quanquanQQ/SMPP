...
class PrototypeBuilder:
    ...
    def build_visual_prototypes(
        ...
        for class_id in range(self.num_classes):
            if class_id not in image_samples or len(image_samples[class_id]) == 0:
                print(f"Warning: No samples found for class {class_id}. Skipping.")
                continue
        ...
        if not visual_prototypes:
            print("Error: No valid visual prototypes could be built. Check your image_samples input.")
            print(f"Debug Info: image_samples={image_samples}")
            raise ValueError("No valid visual prototypes could be built. Check your image_samples input.")

        return torch.stack(visual_prototypes)
    ...
    def sample_diverse_data(
        ...
        if sampling_strategy == "diverse":
            ...
            samples = self._random_sampling(class_samples)
        ...
        # Ensure fixed prototype shot count by topping up with random samples.
        if len(samples) < self.num_shots:
            needed = self.num_shots - len(samples)
            extra = self._random_sampling(class_samples)
            samples = list(samples) + list(extra[:needed])
        elif len(samples) > self.num_shots:
            samples = list(samples[:self.num_shots])
    ...
    def _random_sampling(self, samples):
        """Random sampling with replacement fallback for small classes."""
        n = len(samples)
        if n == 0:
            return []
        if n >= self.num_shots:
            idx = np.random.choice(n, self.num_shots, replace=False)
        else:
            idx = np.random.choice(n, self.num_shots, replace=True)
    ...
    def _temporal_diverse_sampling(self, samples):
        ...
        # 仅使用有时间戳的样本；若不可用则回退随机采样。
        timed_samples = [s for s in samples if s.get('timestamp') is not None]
        if len(timed_samples) < 2:
            return self._random_sampling(samples)

        # 按时间戳排序并分段采样
        sorted_samples = sorted(timed_samples, key=lambda x: x.get('timestamp'))
        if len(sorted_samples) <= self.num_shots:
            return self._random_sampling(sorted_samples)

        # Evenly sample indices across timeline.
    ...
    def _user_diverse_sampling(self, samples):
        ...
        for sample in samples:
            user_id = sample.get('user_id', '__unknown_user__')
        ...
        if len(selected) < self.num_shots:
            ...
            selected.extend(self._random_sampling(samples)[:needed])
