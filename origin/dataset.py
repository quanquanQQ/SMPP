...
class JSONDataset(Dataset):
    ...
    def _align_lengths(self):
        """Align optional metadata lengths with image_paths to prevent index errors."""
        n = len(self.image_paths)

        if len(self.text_data) < n:
            self.text_data.extend({self.text_field: ""} for _ in range(n - len(self.text_data)))
        else:
            self.text_data = self.text_data[:n]

        if len(self.categories) < n:
            self.categories.extend({"Category": "class_0"} for _ in range(n - len(self.categories)))
        else:
            self.categories = self.categories[:n]

        if len(self.popularities) < n:
            self.popularities.extend([0.0] * (n - len(self.popularities)))
        else:
            self.popularities = self.popularities[:n]

        if self.include_user_features:
            if len(self.user_data) < n:
                self.user_data.extend({} for _ in range(n - len(self.user_data)))
            else:
                self.user_data = self.user_data[:n]
    ...
    def _load_data(self):
        ...
        try:
            ...
            print(f"Warning: {self.metadata_dir / f'{self.data_prefix}_img_filepath.txt'} not found. Using placeholder data.")
            self.image_paths = ["placeholder_image_1.jpg", "placeholder_image_2.jpg"]
        ...
        try:
            ...
            print(f"Warning: {self.metadata_dir / f'{self.data_prefix}_text.json'} not found. Using empty text data.")
            self.text_data = []
        ...
        try:
            ...
            print(f"Warning: {self.metadata_dir / f'{self.data_prefix}_category.json'} not found. Generating synthetic categories.")
            self.categories = [{"Category": "class_1"}, {"Category": "class_2"}]  # Synthetic class dicts
        ...
        if self.include_user_features:
            try:
                ...
                print(f"Warning: {self.metadata_dir / f'{self.data_prefix}_user_data.json'} not found. Using empty user data.")
                self.user_data = []
    ...
    def __getitem__(self, idx: int) -> Tuple:
        ...
        # 3. Label (Category)
        cat_item = self.categories[idx] if idx < len(self.categories) else {}
        if not isinstance(cat_item, dict):
            cat_item = {}
        cat_name = cat_item.get('Category', 'class_0')
        label = self.class_mapping.get(cat_name, 0)
        
        # 4. Popularity
        popularity = self.popularities[idx] if idx < len(self.popularities) else 0.0
    ...
    def _sample_prototypes(self) -> dict:
        ...
        for class_id in range(self.num_classes):
            ...
            if len(class_data) < self.num_shots:
                ...
                # 时间多样性采样
                class_data = class_data.sort_values('timestamp')
                step = len(class_data) // self.num_shots
                sampled = class_data.iloc[::step][:self.num_shots]
